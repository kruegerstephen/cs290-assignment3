...
var bar = function(doubleArray){
    ...
    for(i = 0; i<doubleArray.length;i++){
        if(!isNaN(parseFloat(doubleArray[i]))){
            newArrayValue += doubleArray[i]*2;
            originalArray += doubleArray[i];
...
function parseGit(logArray){
    ...
    for(var i = 0; i<logArray.length; i++){
        var logSplit =  logArray[i].split(" ");
        var hash = logSplit[0].toString();
        var date =logSplit[1].toString() +  " " + logSplit[2].toString() +" "+ logSplit[3].toString() +  " " + logSplit[4].toString() + " "+ logSplit[5].toString() + " "+ logSplit[6].toString();
        if(logSplit[9]!=null){
            var message = logSplit[7].toString() + " " + logSplit[8].toString()+ " " + logSplit[9].toString();
        }
        else{
            var message = logSplit[7].toString()+ " " + logSplit[8].toString();
        }
        message = message.replace(/"/g,"");

        newGitLog = new GitLog(hash,date,message);
        gitLogArray[i] = newGitLog;
